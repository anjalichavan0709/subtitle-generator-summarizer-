...
- `backend/app/summary_preprocessor.py`
  ...
  - Applies generic, transcript-driven preprocessing without calculus-specific or MIT/OpenCourseWare-specific hardcoding.
  - Removes filler/source language using reusable cleanup rules.
  - Extracts important lecture sentences and concepts from the transcript itself.
...
The current backend avoids calculus-specific and MIT/OpenCourseWare-specific hardcoding, keeping preprocessing generic and transcript-driven.
...
> The lecture interprets derivatives through rate of change and left hand limit in real situations. It compares average change with instantaneous change and uses one over x, sine x over x, and continuous to show why the distinction matters. Physical and scientific examples connect formulas with measurable quantities such as motion, current, temperature, and uncertainty. The lesson shows how derivatives turn changing data into useful predictions.
...
- WER uses pseudo-reference/generated reference transcripts, so it is not a true ground-truth transcription benchmark.
- Long videos may need chunking or runtime optimization.
- This is a local/code-review version, not a production deployment.
